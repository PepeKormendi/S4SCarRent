CLASS lhc_booking DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS carreturn FOR MODIFY
      IMPORTING keys FOR ACTION booking~carreturn.

    METHODS cartransfer FOR MODIFY
      IMPORTING keys FOR ACTION booking~cartransfer.
...
CLASS lhc_booking IMPLEMENTATION.
  ...
  METHOD calculatebookprice.
    ...
    READ ENTITIES OF zszakd_i_cars IN LOCAL MODE
    ...
    LOOP AT bookings ASSIGNING FIELD-SYMBOL(<booking>).
      ...
      AND <booking>-bookedto GE <booking>-bookedfrom.
        ...
        LOOP AT cars ASSIGNING FIELD-SYMBOL(<car>) .
          ...
          MODIFY ENTITIES OF zszakd_i_cars IN LOCAL MODE
             ENTITY booking
  ...
  METHOD defaultvaluesfunction.
    ...
    DATA bookings_update TYPE TABLE FOR UPDATE zszakd_i_cars\\booking.
...
*      ENDIF.
  ...
  METHOD carreturn.
    ...
    READ ENTITIES OF zszakd_i_cars IN LOCAL MODE
          ENTITY booking
            FIELDS (  customeruuid bookinguuid bookedfrom bookedto bookprice currencycode  )
            WITH CORRESPONDING #( keys )
          RESULT DATA(bookings).


    DATA: ls_input TYPE zautos_invoice_input.

    LOOP AT bookings ASSIGNING FIELD-SYMBOL(<fs_bookings>).
      ls_input = VALUE  #( partnerid = <fs_bookings>-customeruuid
                           bookingid = <fs_bookings>-bookinguuid
                           bookingfrom = <fs_bookings>-bookedfrom
                           bookingto   = <fs_bookings>-bookedto
                           price       = <fs_bookings>-bookprice
                           currency    = <fs_bookings>-currencycode
                            ).
    ENDLOOP.
    SELECT SINGLE * FROM zszakd_cars INTO @DATA(car) WHERE car_uuid = @<fs_bookings>-caruuid.
    ls_input-brand       = CONV #(  car-brand ).
    ls_input-model       = CONV #(  car-model ).
    ls_input-platenumber = CONV #(  car-platenum ).
    IF ls_input-bookingto EQ sy-datum.
      DATA(lc_invoice) = NEW zcl_csz_invoice_helper( is_booking = ls_input ).
      lc_invoice->main( ).
      <fs_bookings>-bookingstatus = '04'.
      MODIFY ENTITIES OF zszakd_i_cars IN LOCAL MODE
       ENTITY booking
         UPDATE FIELDS ( bookingstatus )
         WITH CORRESPONDING #( bookings ).
    ELSE.
      APPEND VALUE #( %tky               = <fs_bookings>-%tky
                      %msg = new_message_with_text(
                       severity = if_abap_behv_message=>severity-error
                       text = 'A foglalás vége nem egyezik meg a mai nap dátumával!' )
                       %element-bookedfrom = if_abap_behv=>mk-on ) TO reported-booking.
    ENDIF.
  ...
  METHOD cartransfer.
    ...
    ls_input-model       = CONV #(  car-model ).
    ls_input-platenumber = CONV #(  car-platenum ).
    IF ls_input-bookingfrom EQ sy-datum.
      DATA(lc_invoice) = NEW zcl_csz_invoice_car_aggrement( is_booking = ls_input ).
      lc_invoice->main( ).
      <fs_bookings>-bookingstatus = '01'.
      MODIFY ENTITIES OF zszakd_i_cars IN LOCAL MODE
       ENTITY booking
         UPDATE FIELDS ( bookingstatus )
         WITH CORRESPONDING #( bookings ).
    ELSE.
      APPEND VALUE #( %tky               = <fs_bookings>-%tky
                            %msg = new_message_with_text(
                             severity = if_abap_behv_message=>severity-error
                             text = 'A foglalás kezd. nem egyezik meg a mai nap dátumával!' )
                             %element-bookedfrom = if_abap_behv=>mk-on ) TO reported-booking.
    ENDIF.
